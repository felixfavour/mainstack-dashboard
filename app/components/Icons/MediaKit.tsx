const MediaKitIcon = () => (<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
  <g style="mix-blend-mode:luminosity">
    <path d="M23.8403 12.925V19.025C23.8403 19.415 23.5303 19.725 23.1403 19.725H17.9803C17.5903 19.725 17.2803 19.415 17.2803 19.025V18.055H12.9803C12.5903 18.055 12.2803 17.745 12.2803 17.355V11.255C12.2803 10.865 12.5903 10.555 12.9803 10.555H18.1403C18.5303 10.555 18.8403 10.865 18.8403 11.255V12.225H23.1403C23.5203 12.225 23.8403 12.545 23.8403 12.925Z" fill="url(#paint0_linear_4386_1458)" />
    <path d="M19.59 22.1849H10.7C10.31 22.1849 10 21.8749 10 21.4849V14.5549C10 14.1649 10.31 13.8549 10.7 13.8549H19.59C19.98 13.8549 20.29 14.1649 20.29 14.5549V21.4849C20.29 21.8749 19.98 22.1849 19.59 22.1849Z" fill="url(#paint1_linear_4386_1458)" />
    <path d="M29.3 29.445H10.7C10.31 29.445 10 29.135 10 28.745V16.475C10 16.085 10.31 15.775 10.7 15.775H29.3C29.69 15.775 30 16.085 30 16.475V28.745C30 29.135 29.69 29.445 29.3 29.445Z" fill="url(#paint2_linear_4386_1458)" />
  </g>
  <defs>
    <linearGradient id="paint0_linear_4386_1458" x1="12.2755" y1="15.1424" x2="23.8365" y2="15.1424" gradientUnits="userSpaceOnUse">
      <stop stop-color="#FCFF1C" />
      <stop offset="1" stop-color="#FF9D0A" />
    </linearGradient>
    <linearGradient id="paint1_linear_4386_1458" x1="10" y1="18.0223" x2="20.293" y2="18.0223" gradientUnits="userSpaceOnUse">
      <stop stop-color="#E7CFFF" />
      <stop offset="1" stop-color="#870FFF" />
    </linearGradient>
    <linearGradient id="paint2_linear_4386_1458" x1="10" y1="22.6143" x2="30" y2="22.6143" gradientUnits="userSpaceOnUse">
      <stop stop-color="#03FFE5" />
      <stop offset="1" stop-color="#14B348" />
    </linearGradient>
  </defs>
</svg>
)

export default MediaKitIcon
